...
<main>
  ...
  <div id="app">
    ...
    <div class="card">
      <h2 style="margin-top:0;font-size:16px;">Pending Registration Requests</h2>
      <table>
        <thead>
          <tr><th>Company</th><th>Subdomain</th><th>Contact</th><th>Submitted</th><th>Actions</th></tr>
        </thead>
...
async function loadPending() {
  const pending = await api('/admin/platform/registration-requests?status=pending&page_size=50');
  const rows = pending.data ?? pending;
  if (!rows.length) {
    $('pending-body').innerHTML = `<tr class="empty-row"><td colspan="5">No pending requests.</td></tr>`;
    return;
  }
  $('pending-body').innerHTML = rows.map((r) => `
    <tr>
      <td>${r.legal_name}</td>
      <td>${r.subdomain}</td>
      <td>${r.admin_email}</td>
...
document.body.addEventListener('click', async (e) => {
  ...
  if (approveId) {
    if (!confirm('Approve this registration and create the tenant account?')) return;
    await api(`/admin/platform/registration-requests/${approveId}/approve`, { method: 'POST' });
    await loadDashboard();
  }

  if (rejectId) {
    const reason = prompt('Reason for rejecting this request (optional):') || null;
    await api(`/admin/platform/registration-requests/${rejectId}/reject`, { method: 'POST', body: JSON.stringify({ reason }) });
    await loadDashboard();
  }
});
</script>
